# Agent Messaging Gateway (AMG) Blueprint
...
* `direction`: `"inbound"` for messages received from a platform, `"outbound"` for messages the agent sent through AMG. Required on every envelope.
...
*Reconciled at v1 acceptance, against `specs/agent-messaging-gateway-SPEC.md` v1.1 and the Phase 4 stability run.*
...
* **Intro** — added a "Source of truth for v1" callout pointing readers at `specs/agent-messaging-gateway-SPEC.md` v1.1 for v1 implementation contracts.
* §5.1 — added the Operator and system endpoints subgroup (`/messages/quarantine`, `/attachments/{id}`, `/healthz`, `/openapi.json`) so the blueprint's REST inventory matches what `amg/api/` actually serves and what spec §7.4 specifies.
...
* `VALIDATION_FAILED` (in code, `amg/core/errors.py`) vs `VALIDATION_ERROR` (in spec §7.4.12). Treat `VALIDATION_FAILED` as canonical pending user decision.
* Default DB filename `state.db` (in code, `amg/core/db.py` and `amg/migrations/env.py`) vs `amg.db` (in spec §11.2 `AMG_DB_PATH` row). Treat `state.db` as canonical pending user decision. Env var name `AMG_DB_PATH` is identical in both surfaces.
