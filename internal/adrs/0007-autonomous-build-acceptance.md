...
- **Phase 1, 2, 3 round-trip acceptance** runs entirely against Phase 0 fakes:
  ...
  - Phase 3 — MCP wrapper round-trip: programmatic `mcp.client.stdio` (Python SDK) **client** harness spawns the wrapper as a stdio subprocess and exercises every tool against the adapter (which is itself wired to the Phase 0 fakes). No MCP Inspector, no Claude Code session.
