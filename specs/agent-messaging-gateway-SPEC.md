...
There is no implementation yet. The repository contains only this PRD's source (the blueprint), README, and Claude Code config. Existing reference work that AMG will build on:
...
- **Without AMG**: every new agent framework explored (Claude Code, Claude Desktop, Codex CLI, Codex Desktop, custom Python loops) requires re-integrating either iMessage or Discord — N×M coupling.
- **With AMG**: one adapter, one MCP wrapper, two connectors. Adding a third platform (Slack, SMS, Telegram) is a single connector. Adding a third agent framework is zero adapter changes.
...
- **Pain Points**: None imposed by AMG; they should not perceive the system except via response latency.
- **Context**: Whatever device they normally message from.
- **Technical Proficiency**: Irrelevant — they never touch AMG directly.
...
The numbering uses `REQ-AMG-NNN`. Priorities: P0 (must ship in v1), P1 (must ship in v1, lower urgency), P2 (post-v1).

---

### 5.1 Feature: Inbound message ingestion (REQ-AMG-001)
...
### 5.2 Feature: Outbound message sending (REQ-AMG-002)
...
### 5.3 Feature: Read state per agent (REQ-AMG-003)
...
### 5.4 Feature: Message context fetch (REQ-AMG-004)
...
### 5.5 Feature: Outbound webhook for new messages (REQ-AMG-005)
...
- [ ] Header `X-AMG-Signature: sha256=<hex>` carries an HMAC of the raw body using a shared secret from `.env`.
- [ ] Header `X-AMG-Delivery-Id: <uuid>` is unique per delivery attempt for idempotent receivers.
- [ ] Header `X-AMG-Message-Id: <id>` mirrors `envelope.id`.
...
| Scenario | Input | Expected Behavior |
|----------|-------|-------------------|
| Receiver returns 200 then crashes before processing | 1 attempt, success per HTTP | Adapter considers it delivered; receiver must do its own idempotency on `X-AMG-Delivery-Id` |
...
### 5.6 Feature: MCP wrapper exposing the four tools (REQ-AMG-006)
...
**Acceptance Criteria**:
- [ ] Wrapper is Python using the official `mcp` SDK (FastMCP), distributed as a uv workspace member at `mcp/` exposing the `amg-mcp` console script invoked via stdio.
...
- [ ] Wrapper config: `AMG_BASE_URL` (default `http://127.0.0.1:8080`), `AMG_BEARER_TOKEN`, `AMG_AGENT_ID`.

**Tool definitions**: see §7.4.7.

---

### 5.7 Feature: Sender allowlist & quarantine (REQ-AMG-007)
...
### 5.8 Feature: Attachment re-hosting (REQ-AMG-008)
...
### 5.9 Feature: Operational endpoints (REQ-AMG-009)
...
- Webhook outbound carries `X-AMG-Signature: sha256=<hex>` HMAC over the raw body; receiver verifies with constant-time compare.
...
- **Secrets**: `~/.config/messaging-agent/.env` (mode `0600`); never logged. Log redactor scrubs `Authorization`, `X-AMG-Signature`, and any field name matching `*_token` / `*_secret`.
...
Not applicable — AMG has no human-facing UI in v1. The `/docs` Swagger page is incidental and auto-generated; accessibility is the responsibility of upstream FastAPI / Swagger UI.
...
- `text` (string): UTF-8.
- `attachments[]`: see REQ-AMG-008. URLs in this list are always adapter-hosted on inbound.
...
- `direction` (`inbound` | `outbound`): which way the message went through AMG.
...
| `direction` | TEXT | NOT NULL CHECK IN ('inbound','outbound') | Direction through AMG |
...
POST {webhook_url}
Content-Type: application/json
X-AMG-Signature: sha256=<hex hmac of body>
X-AMG-Delivery-Id: <uuid, unique per attempt>
...
This section is intentionally empty: AMG is a new product with no prior code to integrate with. See `internal/blueprints/agent-messaging-gateway.md` for the architectural source-of-truth that the spec extends.
...
| AppleScript send has no formal rate limit but has anti-spam heuristics | Bursts can silently fail | Token-bucket per channel (REQ-AMG-002) |
| AppleScript sync is slow (~200–500 ms) | Adds to send P95 | `subprocess` in `to_thread`; budget allows it within 2 s P95 |
| Discord CDN URLs expire | Attachment URLs in raw envelope go dead | Adapter re-hosts (REQ-AMG-008) |
...
| `amg/connectors/imessage/applescript.py` shim | Injectable subprocess interface | Define a thin `AppleScriptSender` protocol, ship a real implementation calling `osascript`, and a `FakeAppleScriptSender` for tests that records every call and returns scripted success/failure | — |
...
| Deliverable | Description | Technical Tasks | Dependencies |
|-------------|-------------|-----------------|--------------|
| Wrapper project | Python 3.12 + official `mcp` SDK (FastMCP) | `mcp/pyproject.toml`, ruff config, console_script `amg-mcp` | — |
...
|-------|-------|-------|-----------------|
| Unit | Envelope normalization (per platform), allowlist filter, HMAC signature, idempotency key dedup, token-bucket math, retry-schedule calculation | pytest, hypothesis (property tests for envelope round-trip) | ≥ 80% line coverage on `amg/core/` |
...
- **Updates**: `git pull` + `uv sync` + `alembic upgrade head` + `launchctl kickstart -k gui/<uid>/com.user.amg-adapter`.
...
| `AMG_WEBHOOK_URL` | Outbound webhook (empty = disabled) | `""` |
| `AMG_WEBHOOK_SECRET` | HMAC shared secret | (required if webhook enabled) |
| `AMG_DISCORD_BOT_TOKEN` | Discord auth | (required if Discord enabled) |
| `AMG_RATE_LIMIT_PER_CHANNEL_RPS` | Token bucket sustained | `1` |
| `AMG_RATE_LIMIT_PER_CHANNEL_BURST` | Token bucket burst | `5` |
| `AMG_ATTACHMENT_RETENTION_DAYS` | Sweeper threshold | `90` |
...
- **Adapter won't start**: check FDA grant, check `.env` exists with `AMG_BEARER_TOKEN`, check allowlist file exists.
...
- **Disk filling from attachments**: lower `AMG_ATTACHMENT_RETENTION_DAYS` and force a sweep; verify sweeper job is running.
...
| OQ-6 | If the `AMG_WEBHOOK_URL` is set but the secret is missing, refuse to start or run without HMAC? | Owner-Operator | Phase 1 | Refuse to start (fail fast); document in `SETUP.md` |
...
| HMAC | Hash-based Message Authentication Code; AMG uses HMAC-SHA256 for webhook body signatures (`X-AMG-Signature`) |
...
| RFC 3339 | The internet-profile of ISO 8601 used for all timestamps in AMG (e.g., `2026-04-25T15:32:11Z`) |
...
| ULID | Universally Unique Lexicographically Sortable Identifier — 26-char base32 ID that sorts by creation time; AMG uses ULIDs (with type prefixes like `msg_`, `att_`) for all primary keys |
...
- Blueprint (source of truth, this spec extends): `internal/blueprints/agent-messaging-gateway.md`
...
These divergences from `internal/blueprints/agent-messaging-gateway.md` must be reconciled in the blueprint as part of Phase 1's checkpoint gate:
...
|---------|------|--------|---------|
| 1.0 | 2026-05-02 | Stephen Sequenzia | Initial version generated by SDD Tools (`create-spec`) interview against the AMG blueprint |
...
| 1.2 | 2026-05-09 | Stephen Sequenzia | MCP wrapper language change: TypeScript (`@modelcontextprotocol/sdk`) → Python (official `mcp` SDK / FastMCP). Wrapper relocated from `mcp-wrapper/` (Node project) to `mcp/` (uv workspace member). All four tool semantics, REST endpoints, env vars (`AMG_BASE_URL` / `AMG_BEARER_TOKEN` / `AMG_AGENT_ID`), error mapping, and acceptance criteria are unchanged. Updated §5.6, §7.2 tech-stack rows, §7.4.7 phrasing, §9.3 Phase-3 deliverables, §10.1 test-strategy rows, §10.7 R-9 risk row, §15.2 glossary, §15.5 references. Rationale: collapse parallel Node/Python toolchains; share `uv` / `ruff` / `pytest`; remove Node prerequisite from operator setup. |
