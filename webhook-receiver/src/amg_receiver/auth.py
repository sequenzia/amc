"""HMAC-SHA256 verification for inbound AMG webhooks.

The adapter signs each delivery as ``X-AMG-Signature: sha256=<hex>`` over
the **raw request body bytes** (see ``amg/core/webhook.py:200-210, 526-527``).
...
def compute_signature(secret: str, body: bytes) -> str:
    """Return the ``sha256=<hex>`` value for ``X-AMG-Signature``."""
...
def verify_signature(secret: str, body: bytes, header_value: str | None) -> bool:
    """Constant-time check of ``X-AMG-Signature`` against ``body``.
