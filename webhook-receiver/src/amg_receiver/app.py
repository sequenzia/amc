"""FastAPI app exposing ``POST /webhook`` for AMG outbound deliveries.
...
2. Verify ``X-AMG-Signature`` against ``AMG_WEBHOOK_SECRET``. Mismatch
   returns ``401`` (a 4xx tells the adapter to dead-letter — correct, since
   signature failures are permanent).
3. Dedupe by ``X-AMG-Delivery-Id``. Duplicate retries return ``204`` and
   skip enqueue.
...
from amg_receiver.auth import verify_signature
...
from amg_receiver.logging import configure_logging

__all__ = ["app", "build_app"]

_log = structlog.get_logger("amg.receiver.app")

_HEADER_SIGNATURE = "X-AMG-Signature"
_HEADER_DELIVERY_ID = "X-AMG-Delivery-Id"
_HEADER_MESSAGE_ID = "X-AMG-Message-Id"
_HEADER_ATTEMPT = "X-AMG-Attempt"


def _hydrate_env_from_dotenv() -> None:
    """Best-effort load of ``~/.config/messaging-agent/.env`` into ``os.environ``.

    Mirrors the adapter's lifespan (``amg/app.py:148-153``) so operators put
...
def build_app(*, bootstrap: bool = True) -> FastAPI:
    ...
    app = FastAPI(
        title="AMG Webhook Receiver",
        version="0.1.0",
        description=(
            "Receives AMG outbound webhooks and runs `claude -p` per inbound "
            "message. All endpoints require the same HMAC signature the adapter "
            "produces (see X-AMG-Signature)."
        ),
    ...
    async def receive_webhook(request: Request) -> Response:
        ...
        if not delivery_id:
            ...
            return JSONResponse(
                {"error": "X-AMG-Delivery-Id required"},
