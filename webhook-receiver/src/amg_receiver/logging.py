...
Mirrors the adapter's pattern (``amg/core/logging.py``) so logs from the
...
``amg`` package) to keep the receiver workspace decoupled. If the rules
...
def configure_logging(
    *,
    ...
    try:
        ...
        fallback = True
        print(
            f"[amg.receiver.logging] log directory unwritable ({log_dir}): {exc}; "
