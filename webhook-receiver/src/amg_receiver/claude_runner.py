...
Each inbound webhook envelope is fed to a fresh ``claude -p`` invocation:
  ...
  * **Permissions** default to the workspace's allowlist for the four
    ``mcp__amg__*`` tools. Setting ``AMG_RECEIVER_DANGEROUS=1`` swaps in
...
_log = structlog.get_logger("amg.receiver.runner")
...
# default when ``ENABLE_TOOL_SEARCH`` is unset — the four ``mcp__amg__*`` tools
# are *deferred* behind the ToolSearch tool and the ``amg`` server reports
...
def build_mcp_config_json(config: ReceiverConfig) -> str:
    ...
    Spawns the AMG MCP wrapper using ``sys.executable -m amg_mcp`` so we
    ...
    PATH-resolution problems that ``"command": "amg-mcp"`` would have under
    ...
    return json.dumps(
        {
            "mcpServers": {
                "amg": {
                    ...
                    "args": ["-m", "amg_mcp"],
                    "env": {
                        "AMG_BEARER_TOKEN": config.bearer_token,
                        "AMG_AGENT_ID": config.agent_id,
                        "AMG_BASE_URL": config.base_url,
                    },
                }
            }
        }
...
def render_user_message(envelope: dict[str, Any]) -> str:
    ...
    The envelope JSON is included verbatim so Claude can call AMG tools
    ...
    return (
        ...
        "the AMG MCP tools (`send_message`, `mark_read`, `get_message_context`). "
