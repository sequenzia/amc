...
Follows the env-driven pattern established in ``amc/core/auth.py`` and
``amc/core/webhook.py``: ``ENV_*`` constants + a frozen dataclass +
``from_env`` classmethod + a module-specific ``ReceiverConfigError``.

Required env vars
-----------------
* ``AMC_WEBHOOK_SECRET`` — shared HMAC-SHA256 secret with the adapter.
  This is the **same** env var the adapter uses, so both sides agree on
  one value.
* ``AMC_BEARER_TOKEN`` — passed through to the MCP wrapper subprocess.

Optional env vars (defaults shown)
----------------------------------
* ``AMC_RECEIVER_BIND_HOST``                ``127.0.0.1``
* ``AMC_RECEIVER_BIND_PORT``                ``8090``
* ``AMC_AGENT_ID``                          ``amc-receiver``
* ``AMC_RECEIVER_AGENT_PROMPT_FILE``        ``~/.config/messaging-agent/agent_prompt.md``
* ``AMC_RECEIVER_DANGEROUS``                ``0`` (set to ``1`` to pass
  ``--dangerously-skip-permissions`` to ``claude``)
* ``AMC_RECEIVER_CLAUDE_TIMEOUT_SECONDS``   ``300``
* ``AMC_RECEIVER_LOG_DIR``                  ``~/Library/Logs/messaging-agent``
* ``AMC_RECEIVER_IDLE_WORKER_TTL_SECONDS``  ``300``
* ``AMC_RECEIVER_DEDUPE_CACHE_SIZE``        ``4096``
* ``AMC_RECEIVER_CLAUDE_BIN``               ``claude``
* ``AMC_BASE_URL``                          ``http://127.0.0.1:8080`` (for the MCP wrapper)
"""
...
ENV_BIND_HOST: Final[str] = "AMC_RECEIVER_BIND_HOST"
ENV_BIND_PORT: Final[str] = "AMC_RECEIVER_BIND_PORT"
ENV_WEBHOOK_SECRET: Final[str] = "AMC_WEBHOOK_SECRET"
ENV_BEARER_TOKEN: Final[str] = "AMC_BEARER_TOKEN"
ENV_AGENT_ID: Final[str] = "AMC_AGENT_ID"
ENV_AGENT_PROMPT_FILE: Final[str] = "AMC_RECEIVER_AGENT_PROMPT_FILE"
ENV_DANGEROUS: Final[str] = "AMC_RECEIVER_DANGEROUS"
ENV_CLAUDE_TIMEOUT_SECONDS: Final[str] = "AMC_RECEIVER_CLAUDE_TIMEOUT_SECONDS"
ENV_LOG_DIR: Final[str] = "AMC_RECEIVER_LOG_DIR"
ENV_IDLE_WORKER_TTL_SECONDS: Final[str] = "AMC_RECEIVER_IDLE_WORKER_TTL_SECONDS"
ENV_DEDUPE_CACHE_SIZE: Final[str] = "AMC_RECEIVER_DEDUPE_CACHE_SIZE"
ENV_CLAUDE_BIN: Final[str] = "AMC_RECEIVER_CLAUDE_BIN"
ENV_BASE_URL: Final[str] = "AMC_BASE_URL"

DEFAULT_BIND_HOST: Final[str] = "127.0.0.1"
DEFAULT_BIND_PORT: Final[int] = 8090
DEFAULT_AGENT_ID: Final[str] = "amc-receiver"
