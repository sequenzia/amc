"""Async SQLAlchemy engine + session factory for the AMC SQLite store.

Spec references: §7.3 (storage), §11.2 (``AMC_DB_PATH``).

Public surface:

- :data:`DEFAULT_DB_PATH` — fallback location when ``AMC_DB_PATH`` is unset.
...
- :func:`create_engine_from_env` — convenience factory that reads
  ``AMC_DB_PATH`` from the environment, applies WAL + ``foreign_keys = ON`` at
...
- The default DB path matches what the spec uses elsewhere
  ...
  in §11.2 lists ``amc.db`` but the in-flight task mandates ``state.db`` to
...
DB_PATH_ENV_VAR: Final[str] = "AMC_DB_PATH"
