...
* ``AMC_RATE_LIMIT_PER_CHANNEL_RPS`` — sustained refill rate (default ``1.0``)
* ``AMC_RATE_LIMIT_PER_CHANNEL_BURST`` — bucket capacity (default ``5``)
...
re-raises this as the canonical ``AMCError(code=RATE_LIMITED, status=429,
...
ENV_RPS: str = "AMC_RATE_LIMIT_PER_CHANNEL_RPS"
ENV_BURST: str = "AMC_RATE_LIMIT_PER_CHANNEL_BURST"
...
class RateLimitedError(Exception):
    ...
    The HTTP boundary translates this into ``AMCError(code=RATE_LIMITED,
...
class RateLimiter:
    ...
    def from_env(cls) -> RateLimiter:
        """Construct a limiter from ``AMC_RATE_LIMIT_PER_CHANNEL_*`` env vars.
