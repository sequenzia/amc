...
override via ``$AMC_ALLOWLIST_PATH``) that declares which iMessage handles and
...
ENV_ALLOWLIST_PATH: Final[str] = "AMC_ALLOWLIST_PATH"
...
def _parse_file(path: Path) -> dict[tuple[Source, str], AllowlistEntry]:
    ...
    if not path.exists():
        raise AllowlistError(
            ...
            f"specs/agent-messaging-channel-SPEC.md §5.7 for the schema) "
...
# ---------------------------------------------------------------------------
# Module-level config cache (parallels amc.core.auth)
# ---------------------------------------------------------------------------
...
def load_allowlist(path: Path | str | None = None) -> AllowlistLoader:
    ...
    2. ``$AMC_ALLOWLIST_PATH`` environment variable.
