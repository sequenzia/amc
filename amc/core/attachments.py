...
* :class:`AttachmentRow` — the value returned by :meth:`AttachmentStore.rehost`,
  ...
  off to :class:`amc.core.message_sink.MessageSink`.
...
_log = structlog.get_logger("amc.attachments")

# Env-var name; mirrored in `.env.example` and spec §11.2.
ENV_ATTACHMENT_DIR: Final[str] = "AMC_ATTACHMENT_DIR"
...
# the spec §11.2 ``AMC_BIND_HOST=127.0.0.1`` / ``AMC_BIND_PORT=8080`` defaults.
        ...
        *,
        ...
        Args:
            store_dir: Override the directory. Falls back to
                ``$AMC_ATTACHMENT_DIR`` then :data:`DEFAULT_STORE_DIR`.
