"""Periodic background sweepers for time-bounded rows in the AMC store.
...
Why a separate module from :mod:`amc.core.idempotency` and
:mod:`amc.core.attachments`?
----------------------------------------------------------
:class:`amc.core.idempotency.IdempotencyStore` already has a
``sweep_expired()`` method that the FastAPI request path can call
opportunistically; :class:`amc.core.attachments.AttachmentStore` owns the
...
* :class:`IdempotencySweeper` — async background task mirroring the
  :class:`amc.core.webhook.WebhookWorker` lifecycle (``start`` /
  ``stop`` / ``tick``).
...
* :class:`AttachmentSweeper` — daily async background task mirroring
  ...
  ``$AMC_ATTACHMENT_RETENTION_DAYS`` (default 90) at construction time.
...
def _format_ts(ts: datetime) -> str:
    ...
    :func:`amc.core.idempotency._format_ts`).
    """
...
class IdempotencySweeper:
    ...
    Mirrors :class:`amc.core.webhook.WebhookWorker`: ``start()`` schedules
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(self._run(), name="amc-idempotency-sweeper")
...
def _resolve_retention_days(override: int | None) -> int:
    ...
    :class:`amc.core.attachments.AttachmentStore` so operators get a single,
...
class AttachmentSweeper:
    ...
    ``$AMC_ATTACHMENT_RETENTION_DAYS`` (default ``90``) unless an explicit
    ...
    async def start(self) -> None:
        ...
        if self._task is not None and not self._task.done():
            return
        self._stop_event.clear()
        self._task = asyncio.create_task(self._run(), name="amc-attachment-sweeper")
        _log.info(
