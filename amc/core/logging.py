...
Log files land in ``$AMC_LOG_DIR`` (default ``~/Library/Logs/messaging-agent``)
...
Source: specs/agent-messaging-channel-SPEC.md §6.2 (Data Protection /
...
def resolve_log_dir(env: Mapping[str, str] | None = None) -> Path:
    ...
    Honors ``AMC_LOG_DIR`` from `env` (falling back to ``os.environ``), then
    ...
    raw = source.get("AMC_LOG_DIR") or DEFAULT_LOG_DIR
...
def configure_logging(
    *,
    ...
    log_dir
        ...
        ``AMC_LOG_DIR`` then the spec default.
    level
    ...
    try:
        ...
        fallback = True
        print(
            f"[amc.logging] log directory unwritable ({resolved_dir}): {exc}; "
...
def install_request_logging(app: Any) -> None:
    ...
    per request.
    """
    log = structlog.get_logger("amc.http")
