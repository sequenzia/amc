...
:class:`amc.core.envelope.Envelope` for persistence. Centralizing the write
path here means:
...
3. ``AMC_WEBHOOK_URL`` is set, surfaced as a non-``None``
   :class:`amc.core.webhook.WebhookConfig` on the sink.
...
:func:`amc.core.db.register_sqlite_pragmas`) keeps readers unblocked.
...
from amc.core.envelope import AllowlistStatus, Direction, Envelope
from amc.core.webhook import WebhookConfig, enqueue_webhook

__all__ = ["MessageSink"]

_log = structlog.get_logger("amc.message_sink")
...
class MessageSink:
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            session_factory: From :func:`amc.core.db.create_session_factory`.
            ...
            webhook_config: ``None`` disables webhook fan-out entirely
                (matches ``AMC_WEBHOOK_URL`` unset). When set, inbound
                allowlisted messages enqueue a delivery row.
            url_snapshot: Pass the running :class:`amc.core.webhook.WebhookWorker`'s
