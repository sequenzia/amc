...
:mod:`amc.api.messages_send` does:
...
:class:`amc.connectors.discord.connector.DiscordConnector` that resolves the
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
iMessage has no OS-level "typing indicator emit" API path that AMC can
...
_log = structlog.get_logger("amc.api.typing")
...
class DiscordTypingClient(Protocol):
    ...
    Implementations resolve the AMC-shaped ``channel_id`` to a Discord
...
class TypingRequest(BaseModel):
    ...
    channel_id: str = Field(
        ...,
        min_length=1,
        description="AMC-shaped channel id (e.g. 'discord:dm:<id>' or '+15551234567').",
    )
...
# Channel id prefix for source inference. Mirrors ``amc.api.messages_send``:
...
def _infer_source(channel_id: str) -> Source:
    ...
    Mirrors :func:`amc.api.messages_send._infer_source`. ``discord:...`` →
