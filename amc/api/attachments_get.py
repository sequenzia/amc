...
local files) into the local store at ``$AMC_ATTACHMENT_DIR/<att_id>`` and
...
matching the pattern in :mod:`amc.api.messages_get` and
:mod:`amc.api.messages_mark_read`.
"""
...
from amc.core.envelope import AttachmentId
...
_log = structlog.get_logger("amc.api.attachments_get")
...
async def get_attachment(attachment_id: _AttachmentIdPath) -> FileResponse:
    ...
    # the source per :class:`amc.core.attachments.AttachmentStore`).
    return FileResponse(
