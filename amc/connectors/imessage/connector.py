...
Owns both halves of the iMessage <-> AMC bridge:

* **Inbound**: a background asyncio task polls
  :class:`amc.connectors.imessage.reader.ChatDbReader` every
  ``poll_interval_seconds`` (default 1.0). Each new ``message.ROWID`` row
  is filtered, normalized into an :class:`amc.core.envelope.Envelope`,
  and handed to :class:`amc.core.message_sink.MessageSink`. The sink's
...
* **Outbound**: :meth:`ImessageConnector.send` resolves the destination
  ...
  :class:`amc.connectors.imessage.applescript.AppleScriptSender`.

Channel id format
-----------------
Per spec §7.3.1 ("iMessage uses E.164 phone or Apple ID email"; no
``imessage:`` prefix) the AMC ``channel_id`` for an iMessage DM is the
...
* **Cursor read fails** (AMC DB unreadable / corrupt): log ERROR and fall
  back to the fresh-install path (max ROWID seed). Treating a transient
  AMC DB error as "fresh install" beats degrading the connector — the
...
:class:`amc.connectors.imessage.reader.ChatDbPathMissingError` at
...
from amc.core.attachments import AttachmentRow, AttachmentStore
...
_logger = logging.getLogger("amc.connectors.imessage.connector")
...
class MessageSinkLike(Protocol):
    ...
    The real implementation is :class:`amc.core.message_sink.MessageSink`;
...
class ImessageConnector:
    ...
    async def start(self) -> None:
        ...
        * **Cursor read failure**: log ERROR and fall back to the
          ...
          on a transient AMC DB hiccup; the next successful sink
    ...
    def _derive_channel_id(self, row: dict[str, Any]) -> str | None:
        """Return the AMC channel_id for an iMessage row.
    ...
    async def _read_cursor_at_startup(self) -> int | None:
        ...
        Raises:
            ``sqlite3.OperationalError`` or ``OSError`` on AMC DB
