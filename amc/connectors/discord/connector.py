...
Owns both halves of the Discord ↔ AMC bridge:
...
* **Inbound**: a ``discord.py`` :class:`discord.Client` runs as an asyncio
  ...
  :class:`amc.core.allowlist.AllowlistLoader`, (2) builds an
  :class:`amc.core.envelope.Envelope`, and (3) hands it to the injected
  :class:`amc.core.message_sink.MessageSink`. Cursor (``connector_state``)
...
* **Outbound**: :meth:`DiscordConnector.send` resolves the
  ...
  for ``reply_to``), and maps ``discord.py`` exceptions to AMC error
  codes:

  * ``discord.Forbidden`` (401/403) → :data:`AmcDiscordErrorCode.PLATFORM_AUTH`
  * any other ``discord.HTTPException`` → :data:`AmcDiscordErrorCode.PLATFORM_SEND_FAILED`
...
_logger = logging.getLogger("amc.connectors.discord.connector")
...
class AmcDiscordErrorCode(StrEnum):
    """Error categories surfaced by :meth:`DiscordConnector.send`.

    These mirror the canonical :class:`amc.core.errors.ErrorCode` values that
    ...
    :class:`amc.core.errors.AMCError`.
    """
...
class SendResult:
    ...
    Attributes:
        ...
        error: One of :class:`AmcDiscordErrorCode` values when ``ok`` is
...
class DiscordConnector(discord.Client):
    """Discord connector — single ``discord.Client`` driving the AMC bridge.
    ...
    Args:
        bot_token: Bot token. In production this is read from
            ``$AMC_DISCORD_BOT_TOKEN`` and passed in by the adapter; tests
    ...
    async def send(
        ...
        *,
        ...
        Args:
            channel_id: AMC-shaped channel id (``discord:dm:<id>`` or
                ``discord:channel:<id>``).
        ...
        if channel is None:
            return SendResult(
                ok=False,
                error=AmcDiscordErrorCode.CHANNEL_NOT_FOUND.value,
        ...
        try:
            ...
            if exc.status == 401:
                self.degraded = True
                code: str = AmcDiscordErrorCode.PLATFORM_AUTH.value
            else:
                code = AmcDiscordErrorCode.PLATFORM_SEND_FAILED.value
            _logger.error(
                "discord_send_failed",
                extra={
                    "channel_id": channel_id,
    ...
    async def on_error(self, event_method: str, /, *args: Any, **kwargs: Any) -> None:
        ...
        same info via the stdlib logger so ``amc.core.logging``'s
