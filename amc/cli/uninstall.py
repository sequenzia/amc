"""Implementation of ``amc uninstall`` (spec §5.4).
...
1. ``launchctl bootout gui/<uid>/<label>`` — tolerates the
   "not bootstrapped" case (exit 3 or 113) which :func:`amc.cli.launchctl.bootout`
...
# The user-LaunchAgents directory where ``amc install`` writes plists.
