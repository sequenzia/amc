"""Service registry for the ``amc`` CLI.

This module is the **single source of truth** for the launchd-managed AMC
...
* Operators may refer to a service by either its short name (``adapter``)
  or its full launchd label (``com.user.amc-adapter``).
...
# must change in lockstep. ``amc doctor`` (spec §5.8) verifies that the
...
class Service:
    """Metadata for one launchd-managed AMC service.

    Fields:
        name: Short, human-friendly identifier (``adapter``, ``receiver``,
            ``backup``). Used as the primary CLI argument.
        label: Full launchd label (``com.user.amc-adapter``). Used to
        ...
        plist_template: Repo-relative path to the plist template under
            ``ops/launchd/``. ``amc install`` reads this, performs
        ...
        app_log_glob: Glob pattern for the structured app-level log files
            ...
            ``adapter-*.log``). Used by ``amc logs`` to find rotated daily
...
# order (spec §5.1). The CLI's own ``--help`` listings and ``amc status``
# output also rely on this ordering.
REGISTRY: tuple[Service, ...] = (
    Service(
        name="adapter",
        label="com.user.amc-adapter",
        plist_template=_OPS_LAUNCHD / "com.user.amc-adapter.plist",
    ...
    Service(
        name="receiver",
        label="com.user.amc-webhook-receiver",
        plist_template=_OPS_LAUNCHD / "com.user.amc-webhook-receiver.plist",
    ...
    Service(
        name="backup",
        label="com.user.amc-backup",
        plist_template=_OPS_LAUNCHD / "com.user.amc-backup.plist",
        # The backup run script lives at ops/scripts/backup.sh per the
        # backup plist's ProgramArguments. It is not yet on disk in v1;
        # ``amc doctor`` will surface its absence (spec §5.8).
...
def resolve_targets(names: list[str]) -> list[Service]:
    ...
    Behavior:
        ...
        * Each name may be either a short name (``adapter``) or a full
          launchd label (``com.user.amc-adapter``).
        ...
        * Mixed inputs work: ``resolve_targets(["adapter",
          "com.user.amc-backup"])`` returns ``[adapter, backup]``.
    ...
    for name in names:
        ...
        # Dedupe on canonical label so ("adapter", "com.user.amc-adapter")
