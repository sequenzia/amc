"""``amc service {start,stop,restart,enable,disable}`` (spec §5.5).
...
``launchctl`` helper from :mod:`amc.cli.launchctl`, prints a per-service
result line via :func:`amc.cli.output.print_status_line`, and returns the
...
* ``1`` — unknown service name (raised by
  :func:`amc.cli.services.resolve_targets`; caught at the command-body
...
imported lazily inside command bodies so ``amc --help`` cold-starts under
200 ms (spec §6.1).
...
# Directory under which installed launchd plist files live. ``amc install``
# writes ``<label>.plist`` here; ``amc service start`` reads it to attempt
...
def _do_start(service: Service) -> int:
    """``amc service start`` for one service.
    ...
    if not plist.exists():
        _emit(
            ...
            "failed",
            ok=False,
            detail="Service not installed; run amc install first",
        )
        return 2
...
def _do_stop(service: Service) -> int:
    """``amc service stop`` for one service.
...
def _do_restart(service: Service) -> int:
    """``amc service restart`` — ``launchctl kickstart -k`` (spec §5.5)."""
...
def _do_enable(service: Service) -> int:
    """``amc service enable`` — ``launchctl enable`` (spec §5.5)."""
...
def _do_disable(service: Service) -> int:
    """``amc service disable`` — ``launchctl disable`` (spec §5.5)."""
...
def _run_verb(
    ...
    :class:`amc.cli.services.UnknownServiceError`, prints the error to
