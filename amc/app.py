...
* Drive the runtime lifespan: load the bearer token, open the SQLite
  ...
  (``AMC_DISCORD_BOT_TOKEN`` set; ``darwin`` host with a readable
...
from amc.connectors.imessage.applescript import OsascriptAppleScriptSender
...
from amc.core.allowlist import load_allowlist
from amc.core.attachments import AttachmentStore
from amc.core.auth import DEFAULT_ENV_PATH, load_bearer_token, require_bearer
...
from amc.core.sweepers import AttachmentSweeper, IdempotencySweeper
from amc.core.webhook import WebhookWorker, load_webhook_config_from_env

__all__ = ["APP_DESCRIPTION", "APP_TITLE", "APP_VERSION", "build_app"]

_log = logging.getLogger("amc.app")
...
def _resolve_bind_url() -> str:
    """Build the externally-visible bind URL from ``AMC_BIND_HOST/PORT``.

    Used as the ``AttachmentStore.bind_url`` so re-hosted attachments
    expose stable absolute URLs in inbound envelopes.
    """
    host = os.environ.get("AMC_BIND_HOST", "").strip() or "127.0.0.1"
    port = os.environ.get("AMC_BIND_PORT", "").strip() or "8080"
...
def build_app(*, bootstrap: bool = True) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        ...
        discord_token = os.environ.get("AMC_DISCORD_BOT_TOKEN", "").strip()
