...
``amg/core/auth.py``. Validation runs only when :func:`load_config` is
...
Per spec §5.6 the wrapper takes three env vars:
  - ``AMG_BASE_URL``     (optional, default ``http://127.0.0.1:8080``)
  - ``AMG_BEARER_TOKEN`` (required)
  - ``AMG_AGENT_ID``     (required)
"""
...
def load_config(env: dict[str, str | None] | None = None) -> WrapperConfig:
    """Read the three ``AMG_*`` env vars, validate, and cache.
