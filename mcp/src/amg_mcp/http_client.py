...
Spec references:
  - §5.6 — wrapper config (AMG_BASE_URL / AMG_BEARER_TOKEN / AMG_AGENT_ID)
...
Design contract:
  ...
  * ``get`` and ``post`` always return a discriminated ``HttpResult``.
    ...
    handlers translate the result via :mod:`amg_mcp.errors`.
...
from amg_mcp.config import WrapperConfig, get_config
