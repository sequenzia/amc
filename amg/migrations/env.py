"""Alembic environment for the AMG SQLite store.

The DB URL is resolved at runtime from ``$AMG_DB_PATH`` (default
...
``aiosqlite`` engine via :mod:`amg.core.db`; the schema is identical.
"""
...
# Make ``amg`` importable when invoked as ``alembic upgrade head`` from the
...
from amg.core.schema import metadata as target_metadata  # noqa: E402

DB_PATH_ENV_VAR = "AMG_DB_PATH"
DEFAULT_DB_PATH = Path("~/Library/Application Support/messaging-agent/state.db").expanduser()


def _resolve_database_url() -> str:
    """Resolve the sync SQLite URL from ``$AMG_DB_PATH``.
