...
* :class:`PrintResult` — the parsed payload of ``launchctl print
  gui/<uid>/<label>``. Fields default to ``"-"`` (matching ``amg status``
...
# graphical-login session domain; every AMG service is a LaunchAgent so this
...
class CompletedResult:
    ...
    Fields:
        ...
        stderr: Captured standard error (text). May be empty. ``amg``
...
class PrintResult:
    ...
    Fields:
        ...
        fields: All parsed ``key = value`` pairs from the print output,
            ...
            output. Exposed for debugging and for ``amg doctor`` which may
...
def _run(argv: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    (``amg.cli.launchctl._run``) when it needs to inject canned output. We
...
def kickstart(label: str, *, restart: bool = False) -> CompletedResult:
    ...
    Used by ``amg service start`` (``restart=False``) and ``amg service
...
def kill_sigterm(label: str) -> CompletedResult:
    ...
    Used by ``amg service stop``. ``SIGTERM`` is the spec-mandated signal
