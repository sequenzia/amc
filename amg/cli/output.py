...
Every command that emits structured output (``amg status``, ``amg doctor``,
``amg service ...``) funnels through this module so format selection — Rich
...
* The module is intentionally lazy-friendly: importing ``amg.cli.output``
  pulls Rich, but ``amg --help`` never imports this module, so the
...
def print_status_line(
    ...
    *,
    ...
    Used by ``amg service start/stop/restart/enable/disable`` to report
