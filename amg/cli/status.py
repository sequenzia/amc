"""``amg status`` — show launchd state for AMG services (spec §5.6).

For each target service, this module:

1. Calls :func:`amg.cli.launchctl.print_service` to get parsed
   ``launchctl print gui/<uid>/<label>`` output (loaded flag, pid,
   last exit, state, start_time).
2. Calls :func:`amg.cli.launchctl.print_disabled_state` **once** to
...
4. Builds a row dict per service (``service``, ``loaded``, ``enabled``,
   ...
   :func:`amg.cli.output.render_table` so the Rich / plain / JSON
...
The module is import-light so ``amg --help`` does not pay for any of
this (spec §6.1) — the command body in :mod:`amg.cli.app` lazy-imports
:func:`run_status`.
"""
...
from amg.cli.launchctl import (
    PrintResult,
    print_disabled_state,
    print_service,
)
from amg.cli.services import (
    REGISTRY,
    Service,
    UnknownServiceError,
    resolve_targets,
...
def _run_ps(pid: str) -> subprocess.CompletedProcess[str]:
    ...
    Centralized so tests can patch :func:`amg.cli.status._run_ps` and
    inject canned output without spinning up a real ``ps`` process.
    Mirrors the ``_run`` seam used by :mod:`amg.cli.launchctl`.
    """
...
def _services_for(target: str) -> list[Service]:
    ...
    Mirrors the pattern used by :mod:`amg.cli.logs` so an empty string
...
def run_status(
    ...
    *,
    ...
    """Execute the ``amg status`` workflow and return the process exit code.
    ...
    # Lazy-import to keep ``amg --help`` cold start under 200 ms
