"""Preflight diagnostics for the ``amg`` install (spec §5.8).

``amg doctor`` is the operator's first-line answer to "why isn't this
...
* **Output via :func:`amg.cli.output.render_table`.** All format selection
...
# Repo root: the package lives at ``<repo>/amg/cli/doctor.py``; two parents up.
...
def check_uv_on_path() -> tuple[str, str]:
    ...
    the installed plists and by ``amg serve``. Missing ``uv`` is a hard
...
def check_plists_installed() -> tuple[str, str]:
    ...
    each missing label so the operator knows which ``amg install`` step
    has not yet been run.
    """
    if not LAUNCH_AGENTS_DIR.is_dir():
        return (
            _FAIL,
            f"{LAUNCH_AGENTS_DIR} does not exist; run 'amg install' to bootstrap services",
        )
    missing: list[str] = []
    for svc in REGISTRY:
...
def check_services_bootstrapped(
    *,
    ...
    :func:`amg.cli.launchctl._run`); tests inject a fake that captures
...
def _default_run(argv: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    (``amg.cli.doctor._default_run``) when it wants to inject canned
...
# ``style`` column option in :func:`amg.cli.output.render_table`.
#
...
def _maybe_color_status(status: str, *, color_enabled: bool) -> str:
    ...
    The plain renderer in :mod:`amg.cli.output` strips no ANSI on its
...
def _output_is_rich(stream: IO[str]) -> bool:
    ...
    :func:`amg.cli.output.render_table`. We replicate them locally so we
...
def _exit_code(results: Iterable[CheckResult]) -> int:
    ...
    should be able to wire ``amg doctor`` into CI gating where
...
def run_doctor(
    *,
    ...
    wrapper in :mod:`amg.cli.app` translates the return into an exit.
...
# amg.cli.doctor import Service` if it wants to swap the registry. We do
