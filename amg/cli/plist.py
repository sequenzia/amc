"""Plist renderer for the ``amg`` CLI.
...
by ``amg install`` (spec §5.3) to render each service's plist template
...
def render_plist(service: Service, *, install_dir: Path, home: Path) -> str:
    ...
    * ``__HOME__``         → ``str(home)``
    ...
    Args:
        ...
        install_dir: Absolute path to the AMG repo root. Substituted for
            ``__INSTALL_DIR__``.
...
def install_plist(text: str, dest: Path) -> None:
    ...
    Args:
        ...
        dest: Final on-disk location, e.g.
            ``~/Library/LaunchAgents/com.user.amg-adapter.plist``.
