"""``amg logs`` — tail-follow AMG service log files (spec §5.7).
...
def run_logs(
    ...
    *,
    ...
    """Execute the ``amg logs`` workflow and return the process exit code.
    ...
    if not resolved_log_dir.is_dir():
        err_stream.write(
            f"Logs directory not found: {resolved_log_dir}. Has any AMG service run yet?\n"
        )
        return 1
    ...
    # "amg logs all" on a fresh box where only adapter has logged yet
...
def _follow_multi(
    ...
    *,
    ...
    needed for ``amg logs all`` where each line must be tagged with the
    originating service.
    """
    if stop is None:
        stop = _never_stop
