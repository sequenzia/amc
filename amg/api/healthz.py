...
# Repo root resolved from this file's location: amg/api/healthz.py → repo/.
...
def load_version(pyproject_path: Path | None = None) -> str:
    ...
    with how :func:`amg.core.auth.load_bearer_token` refuses to start
    without a token.
    """
...
def _resolve_session_factory(request: Request) -> async_sessionmaker:
    ...
    Mirrors the pattern in :mod:`amg.api.messages_context` so the route
