...
The agent posts a :class:`amg.core.envelope.SendMessageRequest`; the route:
...
5. Routes to the configured platform connector:
   * ``discord`` → :meth:`amg.connectors.discord.connector.DiscordConnector.send`.
     ...
     outcome surfaces.
   * ``imessage`` → :meth:`amg.connectors.imessage.connector.ImessageConnector.send`.
...
Attachments policy
------------------
:class:`amg.core.envelope.SendAttachmentInput` permits ``url`` *or* ``path``.
...
* **iMessage** (#51): both ``url`` and ``path`` are honored.
  ...
  * ``url`` must be the adapter-hosted form (``/attachments/<att_id>`` or
    ...
    ``404 MESSAGE_NOT_FOUND`` (matching :mod:`amg.api.attachments_get`
    semantics).
...
* :func:`configure_rate_limiter` — :class:`amg.core.rate_limit.RateLimiter`
  built from env config.
* :func:`configure_discord_connector` — the running
  :class:`amg.connectors.discord.connector.DiscordConnector` instance. May be
...
* :func:`configure_imessage_connector` — the running
  :class:`amg.connectors.imessage.connector.ImessageConnector` instance. May
...
:class:`amg.core.message_sink.MessageSink`. The sink ties cursor advancement to
...
from amg.core.envelope import SendAttachmentInput, SendMessageRequest, Source
from amg.core.errors import AMGError, ErrorCode
from amg.core.idempotency import IdempotencyContext, idempotent
from amg.core.rate_limit import RateLimitedError, RateLimiter
...
_log = structlog.get_logger("amg.api.messages_send")
...
def _infer_source(channel_id: str) -> Source:
    ...
    the inverse of :func:`amg.connectors.discord.connector.build_channel_id`.
    """
...
async def _resolve_attachment_bytes_path(
    ...
    if row is None or row["bytes_path"] is None:
        # Mirror :mod:`amg.api.attachments_get` — a deleted-row and a
...
def _extract_reply_to_for_discord(reply_to: str | None) -> str | None:
    """Translate the AMG ``msg_<ULID>`` reply-to to a Discord snowflake.

    The Discord connector's ``send(reply_to=...)`` expects a Discord message
    snowflake. AMG ``reply_to`` is the canonical AMG message id (``msg_<ULID>``),
    ...
    ``amg/connectors/discord/connector.py:on_message`` -- ``reply_to=None``).
...
def _map_discord_error(result: DiscordSendResult, channel_id: str) -> AMGError:
    """Translate a connector :class:`SendResult` failure to an :class:`AMGError`."""
    detail = {"channel_id": channel_id, "platform_detail": result.detail}
    if result.error == AmgDiscordErrorCode.PLATFORM_AUTH.value:
        return AMGError(
