...
* :class:`AMGError` — the canonical exception. Routes raise this instead of
...
* :func:`register_exception_handlers` — wires the three handlers
  (``AMGError``, ``RequestValidationError``, ``Exception``) into a FastAPI
  app.

Translating subsystem-specific errors
-------------------------------------
The rate limiter raises :class:`amg.core.rate_limit.RateLimitedError` with a
...
exception at the route boundary, build an ``AMGError`` with the matching
...
# ``status`` to ``AMGError`` to override (e.g. a connector-specific 503).
...
def _amg_error_to_response(exc: AMGError) -> JSONResponse:
...
async def _amg_error_handler(request: Request, exc: AMGError) -> JSONResponse:  # noqa: ARG001
    return _amg_error_to_response(exc)
...
async def _unhandled_exception_handler(
    ...
    """
    log = structlog.get_logger("amg.errors")
    log.error(
        "unhandled_exception",
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    """
    app.add_exception_handler(AMGError, _amg_error_handler)
