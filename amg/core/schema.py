"""SQLAlchemy Core schema for the AMG SQLite store (spec §7.3.2 / §7.3.3).

This module defines :data:`metadata` containing all nine tables, indexes, and
``CHECK`` constraints exactly as specified in the AMG spec. It is consumed by:

- The Alembic migration ``001_init`` (calls ``metadata.create_all``).
- The async engine in :mod:`amg.core.db` (used at runtime for queries).
...
- Self-referential and forward-referencing FKs (``messages.reply_to`` and
  ...
  :mod:`amg.core.db` does at every connection bootstrap.
