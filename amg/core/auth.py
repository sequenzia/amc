"""Bearer-token authentication and per-agent identification for the AMG adapter.
...
``AMG_BEARER_TOKEN`` is missing or empty, the loader raises :class:`RuntimeError` so
...
_TOKEN_ENV_VAR: Final[str] = "AMG_BEARER_TOKEN"
...
def load_bearer_token(env_path: Path | None = None) -> str:
    """Load ``AMG_BEARER_TOKEN`` and cache it for :func:`require_bearer`.
    ...
    if not token:
        raise RuntimeError(
            f"{_TOKEN_ENV_VAR} is not set. The AMG adapter refuses to start without a "
