...
X-AMG-Signature: sha256=<hex-hmac-of-raw-body>
X-AMG-Delivery-Id: <uuid>
```
...
2. **Verify the signature.** `X-AMG-Signature: sha256=<hex>` is checked with a
   constant-time HMAC-SHA256 over the raw body using `AMG_WEBHOOK_SECRET`. On
   mismatch the receiver returns `401`.
3. **Dedupe on `X-AMG-Delivery-Id`** using a bounded LRU. An already-seen
...
sequenceDiagram
    ...
    Adapter->>Receiver: POST /webhook (signed, X-AMG-Delivery-Id)
    ...
    alt signature invalid
        ...
        Claude->>Adapter: AMG MCP tools (read context, reply)
    end
```
...
- A per-channel worker is **evicted after an idle TTL**
  (`AMG_RECEIVER_IDLE_WORKER_TTL_SECONDS`, default 300s) so idle channels do
  not accumulate.
...
- **System prompt** — loaded from `~/.config/messaging-agent/agent_prompt.md`
  (override with `AMG_RECEIVER_AGENT_PROMPT_FILE`), falling back to a bundled
  default. The file is re-read on every invocation, so prompt edits take
  effect without restarting the service.
- **`--mcp-config`** — points at the AMG MCP wrapper, invoked as
  `<python> -m amg_mcp` from this venv (so PATH resolution under launchd is not
  a concern). The wrapper receives `AMG_BEARER_TOKEN`, `AMG_AGENT_ID`, and
  `AMG_BASE_URL` via this config.
...
- **`--allowedTools`** — limited to the four AMG MCP tools
  (`list_unread_messages`, `send_message`, `mark_read`, `get_message_context`).
  When `AMG_RECEIVER_DANGEROUS=1`, this is replaced by
...
| Variable | Notes |
| --- | --- |
| `AMG_WEBHOOK_SECRET` | Shared HMAC secret — **must match the adapter's**. |
| `AMG_BEARER_TOKEN` | Adapter bearer token, passed through to the MCP wrapper. |

**Notable optional (defaults shown):**

| Variable | Default | Notes |
| --- | --- | --- |
| `AMG_RECEIVER_BIND_HOST` | `127.0.0.1` | Bind interface. |
| `AMG_RECEIVER_BIND_PORT` | `8090` | Bind port. |
| `AMG_AGENT_ID` | `amg-receiver` | Per-agent cursor identity for the MCP wrapper. |
| `AMG_RECEIVER_AGENT_PROMPT_FILE` | `~/.config/messaging-agent/agent_prompt.md` | System prompt source. |
| `AMG_RECEIVER_DANGEROUS` | `0` | `1` swaps in `--dangerously-skip-permissions`. |
| `AMG_RECEIVER_CLAUDE_TIMEOUT_SECONDS` | `300` | Per-message wall-clock budget. |
| `AMG_RECEIVER_CLAUDE_BIN` | `claude` | Path to the `claude` binary. |
| `AMG_BASE_URL` | `http://127.0.0.1:8080` | Adapter HTTP base for the MCP wrapper. |

!!! danger "Secrets must match exactly"
    The receiver's `AMG_WEBHOOK_SECRET` must **exactly** match the adapter's.
...
the [amg CLI](../reference/cli.md):

```bash
amg install receiver
amg service start receiver
```

### Enabling push on the adapter

The receiver only does anything if the **adapter** is configured to push to
it. On the adapter side, set `AMG_WEBHOOK_URL` (and the matching
`AMG_WEBHOOK_SECRET`) to point at this receiver, typically in
`~/.config/messaging-agent/.env`:

```bash
AMG_WEBHOOK_URL=http://127.0.0.1:8090/webhook
AMG_WEBHOOK_SECRET=<same secret the receiver uses>
```
