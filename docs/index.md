...
!!! note "One agent, two platforms"
    AMG normalizes iMessage and Discord so an agent works the same way on both: see what's new, look up context, reply, mark done. Adding a third platform means writing one connector that emits the standard envelope — nothing upstream changes.
...
AMG is built as three independently replaceable layers. See the [Architecture overview](architecture/index.md) for the full design.
...
| [Reference: amg CLI](reference/cli.md) | `serve`, `install`, `service`, `status`, `logs`, `doctor`. |
| [Operations: Overview](operations/index.md) | Running AMG under `launchd` on a single Mac. |
